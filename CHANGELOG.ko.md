# 변경 이력

## 0.6.2

- Setup Sync에서 Git user name 또는 email이 없으면 입력받아 Programmers storage 저장소에만 설정하도록 했습니다.
- Setup Sync에서 입력한 token이 기존 system credential보다 우선하도록 하고, system Git credentials로 전환할 때 저장된 token을 삭제하도록 했습니다.
- token 선택이나 입력을 취소했을 때 이전에 저장된 token으로 계속 진행하지 않고 Setup Sync를 중단하도록 했습니다.
- 변경된 Git identity와 인증 동작을 문서에 반영했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.6.2`로 올렸습니다.

## 0.6.1

- Programmers 문제 본문에 중첩된 HTML `div`가 있는 경우 `problem.md`가 중간에서 잘리던 문제를 수정했습니다.
- 입출력 예 표의 첫 컬럼이 `no`, `case`, `번호` 등 테스트 케이스 라벨이면 입력값에서 제외하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.6.1`로 올렸습니다.

## 0.6.0

- 문제 목록 갱신 흐름을 캐시된 상태 중심으로 정리했습니다. 다시풀 체크, 현재 문제 인덱스 갱신, 삭제, 명령 fallback 목록에서 `problem-index.json`을 읽기 전에 메모리 문제 목록을 먼저 사용합니다.
- 다시풀 checkbox 변경은 Webview에서 즉시 반영하고, 문제별 최신 상태만 짧게 모아 저장합니다. 저장 후에는 이미 알고 있는 review 값으로 인덱스를 patch해 각 문제를 다시 읽지 않습니다.
- 문제 목록 row를 안정적으로 재사용하면서 작은 문제 번호, 난이도, 풀이 기록 수를 함께 표시하고, 다시풀 변경 중 스크롤 위치를 유지합니다.
- 메모/웹 열기 동작은 현재 열린 문제 정보를 먼저 재사용하고, 타이머도 메모리 상태를 우선 사용한 뒤 필요할 때만 `timer.json`을 읽습니다.
- 문제 목록 새로고침 버튼은 명시적인 전체 재스캔으로 취급해 `problem-index.json`을 다시 만들고, 일반 사이드바 복원은 메모리 캐시, index JSON, 파일시스템 scan 순서로 fallback합니다.
- 캐시에 남은 문제 row가 외부에서 삭제된 폴더를 가리킬 때 새로고침 안내가 포함된 메시지를 표시합니다.
- `package.json`과 `package-lock.json` 버전을 `0.6.0`으로 올렸습니다.

## 0.5.6

- C++ sanitizer 디버그 실행 파일을 임시 산출물로 취급해 debug fingerprint cache를 만들지 않고, 재시도 실행이 끝나면 `cpp-debug`를 삭제합니다.
- C++ debug 재시도 빌드에서 `-g`를 제거해 macOS debug symbol bundle 생성을 피하고 임시 산출물 크기를 줄였습니다.
- Docker 런타임 컨테이너를 네트워크 비활성 상태로 생성해 테스트 중 제출 코드가 외부 네트워크에 접근하지 못하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.6`으로 올렸습니다.

## 0.5.5

- `Programmers: Store GitHub Token` 명령을 추가해 remote URL과 branch를 다시 설정하지 않고도 HTTPS GitHub token을 저장하거나 교체할 수 있게 했습니다.
- `Programmers: Show Sync Info`에 현재 인증 경로를 나타내는 `Auth` 정보를 추가했습니다. 저장된 token, system Git credentials, SSH key/agent, 미설정 상태를 구분합니다.
- `Programmers: Sync Now` 실행 시 자동으로 sync info를 Output에 출력하지 않도록 정리했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.5`로 올렸습니다.

## 0.5.4

- 기본 `Run Extension` 디버그 설정이 분리된 `/tmp` user data, extensions, workspace 폴더를 사용하도록 바꿔 개발판과 배포판 상태가 섞일 가능성을 줄였습니다.
- `Programmers: Show Sync Info` 명령을 추가해 현재 sync storage, branch, remote URL을 확인할 수 있게 했습니다.
- `Programmers: Sync Now` 실행 시 Output에 sync 대상 storage, branch, remote URL을 기록하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.4`로 올렸습니다.

## 0.5.3

- 개발판과 배포판이 같은 VS Code user data를 쓰더라도 Git sync remote URL이 섞이지 않도록 remote URL 저장 키를 개발판/배포판으로 분리했습니다.
- 개발판에서는 기존 legacy `programmersHelper.sync.remoteUrl` 설정을 fallback으로 읽지 않게 해 배포판 또는 오래된 설정이 개발판 `Programmers` 저장소의 `origin`을 바꾸는 문제를 막았습니다.
- 확장 시작 로그에 실제로 선택된 `Programmers` storage 경로를 출력해 개발판 workspace 저장소와 배포판 globalStorage 저장소를 구분하기 쉽게 했습니다.
- 자동 업데이트 후 reload 안내 감지 코드를 제거하고 관련 작업자 문서를 정리했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.3`으로 올렸습니다.

## 0.5.2

- 같은 문제 안에서 현재 풀이와 풀이기록을 오갈 때 왼쪽 에디터는 `problem.md`로 유지하고, 오른쪽 풀이/풀이기록 탭은 닫지 않고 보존하도록 했습니다.
- 에디터 탭 정리 방식을 고르는 `programmersHelper.tabResetMode` 설정을 추가했습니다. `always`, `onProblemChange`, `never` 모드를 지원합니다.
- Settings UI 설정 항목을 General, Test Runner, Sync 그룹으로 나누고 순서와 설명을 정리했습니다.
- README에 탭 reset 모드 설명을 추가했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.2`로 올렸습니다.

## 0.5.1

- Marketplace 배포를 태그 push release workflow에서 분리해 수동 실행 GitHub Actions workflow로 옮겼습니다.
- tag 입력이 없으면 최신 GitHub Release, 입력이 있으면 해당 Release에 첨부된 VSIX를 받아 Marketplace에 배포하도록 했습니다.
- 개발 모드에서 `Programmers` 저장소 폴더를 고를 때 extension source 폴더보다 열린 workspace를 우선하도록 수정했습니다.
- Sync Now의 원격 통합 방식을 rebase에서 merge로 바꿔, 로컬 커밋마다 같은 충돌을 반복 해결할 가능성을 줄였습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.1`로 올렸습니다.

## 0.5.0

- Programmers 저장소 폴더를 Git으로 동기화하는 선택 기능을 추가했습니다. Setup Sync, Sync Now, 저장소 폴더 열기, 저장된 GitHub token 삭제 명령을 포함합니다.
- HTTPS GitHub token은 VS Code SecretStorage에 저장하고, Git 명령 인자에 직접 넣지 않고 askpass 스크립트를 통해 전달합니다.
- 활성화 시 pull-only 자동 반영과 commit/pull/push 없이 동기화 필요 여부만 확인하는 주기적 상태 확인 설정을 추가했습니다.
- Git merge/rebase 충돌을 처리하는 conflict resolver Webview를 추가하고, 삭제/수정 충돌은 문제 폴더 단위로 유지 또는 삭제를 선택할 수 있게 했습니다.
- pre-commit, pre-push conflict marker guard hook을 설치하고, 생성 runner artifact와 생성된 problem index는 Git에서 제외하도록 정리했습니다.
- `src` 코드를 `core`, `problems`, `runners`, `sync`, `timers`, `ui` 폴더로 재구성했습니다.
- 문제 명령을 CRUD, 풀이, 테스트 action controller로 나누고, 테스트 러너를 runtime, compiler, executor controller로 분리했습니다.
- `npm run check`가 중첩 JavaScript 파일까지 검사하고 최근 sync controller 회귀 유형을 더 빨리 잡도록 보강했습니다.
- Git sync 설정, Sync Now 동작, 충돌 처리, 동기화 대상 파일 정책을 README와 작업자 문서에 반영했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.5.0`으로 올렸습니다.

## 0.4.3

- Docker 모드에서 마지막 문제를 자동 복원할 때 런타임 컨테이너를 바로 시작하지 않고, 사용자가 문제를 열거나 테스트를 실행할 때 준비한다는 대기 상태를 표시합니다.
- 커스텀 테스트 영역의 안내 문구를 줄이고, Docker 컨테이너가 VS Code 종료 시 자동 정리된다는 안내를 추가했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.4.3`으로 올렸습니다.

## 0.4.2

- 확장이 처음 활성화될 때 마지막으로 사용한 문제를 자동으로 다시 열고 준비해, VS Code 재시작 후에도 사이드바 상태가 현재 로컬 또는 Docker 실행 준비 상태를 반영하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.4.2`로 올렸습니다.

## 0.4.1

- 문제 생성 중에는 입력창과 생성 버튼을 잠그고, 확장 쪽에서도 동시 생성 요청과 짧은 연속 요청을 제한합니다.
- 문제 목록에서 현재 열린 문제를 배경, 왼쪽 표시선, `현재` 배지로 구분합니다.
- 문제 목록의 필터, 새로고침, 검색 영역은 고정하고 문제 row만 스크롤되도록 했습니다.
- 목록 필터를 바꿀 때 현재 문제나 선택된 풀이 스냅샷으로 이동하되, 다시풀 토글이나 삭제 후에는 스크롤을 빼앗지 않도록 했습니다.
- 풀이기록 목록에서 선택한 스냅샷을 표시하고, 사이드바 갱신 후에도 선택 상태를 유지합니다.
- 사이드바를 다시 열 때 문제 목록 스크롤 위치를 복원하고, 스크롤이 멈춘 뒤 위치를 저장합니다.
- 문제 목록과 풀이기록 row에서 가로 스크롤이 생기지 않도록 정리했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.4.1`로 올렸습니다.

## 0.4.0

- Dev Container와 symlink 기반 개발 흐름을 제거하고 로컬 VS Code `Run Extension`을 개발 경로로 정리했습니다.
- 개발용 Extension Host가 현재 workspace를 열도록 해 개발 중 만든 문제 파일이 `Programmers/` 아래에 유지되도록 했습니다.
- VS Code 공식 extension mode 기준으로 개발판과 패키징 설치본을 구분하도록 정리했습니다.
- 개발 모드에서는 업데이트 재로드 알림을 표시하지 않고, 패키징 설치본에서는 기존 재로드 알림을 유지합니다.
- Programmers 페이지 요청에서 `502`, `503`, `504`와 일시적인 네트워크 오류가 발생하면 짧게 재시도하도록 했습니다.
- 존재하지 않거나 접근할 수 없는 페이지처럼 확정적인 HTTP 실패는 즉시 오류로 표시합니다.
- `package.json`과 `package-lock.json` 버전을 `0.4.0`으로 올렸습니다.

## 0.3.9

- Dev Container 개발 흐름을 제거하고 로컬 `Run Extension`을 개발 경로로 정리했습니다.
- 개발용 Extension Host가 현재 workspace를 열도록 해 개발 중 만든 문제 파일이 `Programmers/` 아래에 유지되도록 했습니다.
- VS Code 공식 extension mode 기준으로 개발판과 패키징 설치본을 구분하도록 정리했습니다.
- 업데이트 재로드 알림 제외 조건을 VS Code 공식 개발 모드 기준으로 단순화했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.9`로 올렸습니다.

## 0.3.8

- Marketplace 배포판을 제거한 뒤에도 Programmers 사이드바가 다시 보이도록 Dev Container의 로컬 확장 symlink를 VS Code Server 확장 프로필에 다시 등록합니다.
- 로컬 개발 symlink를 재생성할 때 확장의 오래된 obsolete 표시를 함께 정리합니다.
- 개발 모드와 로컬 symlink 설치에서는 업데이트 재로드 알림을 건너뛰고, 패키징된 설치본에서는 기존 알림을 유지합니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.8`로 올렸습니다.

## 0.3.7

- 타이머 저장 시간을 초 단위로 내림해 중지 시 표시 시간이 예상보다 1초 늘어나는 느낌을 줄였습니다.
- 실행 중 checkpoint는 초 미만 나머지를 `startedAt`에 유지해 장시간 실행 중 시간이 누락되지 않도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.7`로 올렸습니다.

## 0.3.6

- 자동 업데이트로 새 확장 버전이 설치되면 VS Code 재로드 안내를 표시하고, 사용자가 선택하면 창을 즉시 다시 로드하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.6`으로 올렸습니다.

## 0.3.5

- 타이머 저장 위치를 workspace 저장소에서 `.programmers-helper/timer.json`으로 옮기고, 30초 checkpoint와 기존 타이머 상태 마이그레이션을 추가했습니다.
- 종료 3/2/1분 전과 타임오버 알림을 1회씩 표시하고, 사이드바 토글 후에도 타이머 상태를 복원하며, 중지된 타이머를 별도 색상으로 표시합니다.
- 커스텀 테스트 저장 버튼과 저장됨/저장 필요 표시를 추가하고, 임시 타이핑 입력값은 Webview state에 저장하지 않도록 정리했습니다.
- 최신 상태 메시지를 extension host에서 보관해 사이드바를 다시 열 때 복원하고, 문제별 작업 상태에는 문제 폴더명을 함께 표시합니다.
- 현재 문제 액션 위치가 밀리지 않도록 상태 영역을 제한하고 긴 메시지는 박스 내부에서 스크롤되도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.5`로 올렸습니다.

## 0.3.4

- 문제별 풀이 타이머를 추가하고, 사용자가 직접 시작/중지/초기화할 수 있게 했습니다.
- 목표 시간을 30/60/90/120분 중 선택할 수 있게 하고, 목표 시간을 넘기면 타이머 표시를 강조합니다.
- 타이머 상태를 workspace 저장소에 보관하며, 문제 전환이나 확장 종료 시 실행 중인 타이머를 정산 후 중지합니다. 타이머 UI는 실행 중일 때만 1초마다 갱신합니다.
- 문제 번호 입력칸 오른쪽에 `생성 및 열기` 버튼을 배치하고, 사이드바의 `마지막 열기` 버튼을 제거했습니다.
- 준비 완료/실패 상태 박스 높이를 고정해 긴 메시지가 전체 레이아웃을 밀지 않고 박스 내부에서 스크롤되도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.4`로 올렸습니다.

## 0.3.3

- `.programmers-helper` 내부를 `initial`, `generated/runners`, `generated/artifacts`, `generated/fingerprints` 영역으로 나눠 정리했습니다.
- 기존 문제 폴더의 초기 코드와 URL fallback은 유지하면서 `programmers.json`의 중복 메타데이터를 줄였습니다.
- Python 테스트 실행 중 `__pycache__`가 생기지 않도록 하고, C++/Java/Python 생성물이 사용자 풀이 파일 주변에 섞이지 않도록 정리했습니다.
- 새 helper 저장 구조와 메타데이터 소유 규칙을 작업자 문서에 반영했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.3.3`으로 올렸습니다.

## 0.3.2

- 로컬 실행 모드에서 문제를 열 때 C++/Java/Python 실행 명령어를 확인하고 준비 완료 또는 준비 실패 상태를 표시하도록 했습니다.
- Docker 실행과 로컬 실행의 필요 조건을 구분해 문서화하고, local 명령어 준비 확인 흐름을 작업자 문서에 반영했습니다.

## 0.3.1

- Docker 실행 컨테이너 준비 중 사이드바 진행 표시와 단계별 상태 메시지를 함께 보여주도록 개선했습니다.
- 런타임 이미지 캐시가 있어도 실제 Docker image 존재 여부를 다시 확인하고, 이미지가 없으면 항상 Docker Hub pull을 먼저 시도한 뒤 실패할 때만 로컬 build fallback을 사용하도록 했습니다.

## 0.3.0

- Python 템플릿 fetch, `solution.py`/`test_runner.py` 생성, `python3` 기반 샘플 및 커스텀 테스트 실행을 추가했습니다.
- Python은 별도 컴파일 없이 생성된 runner가 `importlib`로 사용자 풀이를 로드해 `solution(...)`을 호출합니다.
- helper Docker 런타임을 `kangjung/programmers-helper-runtime:3`에서 먼저 pull하고 실패하면 `programmers-helper-runtime:3` 로컬 build로 fallback하도록 했습니다.
- Python traceback에서 `solution.py` 사용자 코드 위치와 주요 예외를 요약하도록 런타임 오류 표시를 보강했습니다.

## 0.2.3

- `VSCE_PAT` repository secret을 사용해 태그 배포 시 GitHub Actions에서 VS Code Marketplace까지 자동 publish하도록 했습니다.
- Release 패키징 전에 `package.json` 버전과 `v*.*.*` 태그 버전이 일치하는지 검증하도록 했습니다.
- 개발 문서에 자동 Marketplace 배포 secret 안내를 추가했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.2.3`으로 올렸습니다.

## 0.2.2

- Marketplace 배포를 위해 공개 publisher ID, 패키지 아이콘, 배너 색상, 검색 키워드 메타데이터를 정리했습니다.
- README에 Marketplace 설치 안내, 업데이트 방식, 개인정보 안내, 면책 문구를 반영했습니다.
- 로컬 문제 풀이 작업 폴더와 개발 전용 파일이 VSIX에 포함되지 않도록 패키징 제외 목록을 보강했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.2.2`로 올렸습니다.

## 0.2.1

- 현재 활성 풀이 파일 코드를 클립보드에 복사하고 프로그래머스 원문 페이지를 여는 `웹` 버튼을 추가했습니다.
- 개발용 Dev Container에 기본 JDK를 설치해 개발판 local 실행 모드에서도 Java 실행이 가능하도록 했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.2.1`로 올렸습니다.

## 0.2.0

- `programmersHelper.language` 설정을 추가하고 C++/Java 풀이 파일, 초기 템플릿, 풀이 기록을 언어별로 관리하도록 확장했습니다.
- Java 템플릿 fetch, `Solution.java`/`TestRunner.java` 생성, `javac`/`java` 기반 샘플 및 커스텀 테스트 실행을 추가했습니다.
- 언어별 runner builder와 에러 진단 파싱 구조를 도입해 이후 지원 언어를 추가하기 쉽게 정리했습니다.
- 풀이기록에서 현재 언어 기록을 먼저 보여주고 다른 언어 기록은 접힌 섹션으로 분리해 볼 수 있게 했습니다.
- helper Docker 런타임을 C++/Java 도구와 UTF-8 locale을 포함한 `programmers-helper-runtime:2`로 갱신해 한글 문제 경로의 Java 컴파일을 지원합니다.
- `package.json`과 `package-lock.json` 버전을 `0.2.0`으로 올렸습니다.

## 0.1.1

- Windows 로컬 실행에서 생성된 C++ 테스트 러너가 `.exe` 바이너리로 빌드되도록 수정했습니다.
- 로컬 테스트 러너에서는 POSIX 전용 메모리 측정 코드를 생성하지 않아 Windows 빌드가 `sys/resource.h`를 포함하지 않도록 했습니다.
- Windows에서는 Docker 실행 모드를 권장하고, 로컬 실행에는 MinGW `g++` 또는 LLVM `clang++`가 필요하다는 안내를 추가했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.1.1`로 올렸습니다.

## 0.1.0

- 기본 키보드 단축키를 추가했습니다: 샘플 테스트 실행은 `Ctrl+Alt+T`, 실행 중지는 `Ctrl+Alt+S`입니다.
- 샘플 테스트 실행, 실행 중지, 커스텀 테스트 실행, 메모 열기를 VS Code 명령으로 노출해 사용자가 원하는 키를 지정할 수 있게 했습니다.
- 문제 번호 입력창에서 `Enter`를 누르면 `문제 생성 및 열기` 버튼과 같은 동작을 하도록 했습니다.
- 문제 열기, 리뷰 변경, 삭제, 풀이기록 변경 후 사이드바 문제 목록 캐시를 무효화하지 않고 갱신된 인덱스 목록으로 바로 교체하도록 개선했습니다.
- Webview 렌더링에서 문제 목록과 풀이기록 row를 key 기반으로 재사용해 HTML 재파싱과 DOM 재생성을 줄였습니다.
- 주요 JavaScript 모듈에 책임을 드러내는 주석을 보강하고, store, runner, Docker, parsing, rendering 흐름을 더 작은 책임 단위로 정리했습니다.
- `package.json`과 `package-lock.json` 버전을 `0.1.0`으로 올렸습니다.

## 0.0.18

- 이전 `0.0.17` 배포 때 누락된 manifest 버전 반영을 바로잡아 `package.json`과 `package-lock.json`을 `0.0.18`로 올렸습니다.
- `programmersHelper.executionMode`가 `docker`에서 `local`로 바뀌면 helper Docker 컨테이너를 자동으로 중지하고, Docker 정리 명령에 제한 시간을 두며 사이드바에 결과를 표시합니다.
- `local`에서 `docker`로 다시 바뀔 때는 가벼운 전환 안내만 표시하고, Docker 컨테이너는 다음 문제 열기나 테스트 실행 시점에 준비합니다.
- 활성 C++ 테스트 대상을 `solution.cpp`와 저장된 `solution-*.cpp` 스냅샷으로 제한해 생성된 `test_runner.cpp`가 실수로 실행되지 않도록 했습니다.
- 문제 레이아웃 정리 중 현재 `Programmers` 루트 밖의 관련 없는 C++ 탭이 닫힐 수 있는 경로 계산을 수정했습니다.
- 문제 목록 인덱스와 풀이 스냅샷 경로를 사용하기 전에 검증해, 오래된 인덱스는 재생성하고 안전하지 않은 스냅샷 메타데이터는 무시합니다.
- 사이드바 문제 검색 렌더링에 debounce를 적용해 연속 입력 중 불필요한 DOM 재생성을 줄였습니다.
- `docs/feature-flows.md`에 실행 모드 변경, 실행 대상 선택, 문제 인덱스, 스냅샷 경로 안전성 흐름을 보강했습니다.

## 0.0.17

- 초기화나 새 풀이 시작 시 오른쪽 풀이 에디터를 안정적으로 유지해 화면 깜빡임을 줄였습니다.
- 저장된 풀이 스냅샷을 테스트할 때 선택된 소스 include 경로를 컴파일 fingerprint에 포함해 오래된 바이너리가 재사용되지 않도록 수정했습니다.
- 문제 전환 후 현재 Markdown 미리보기와 C++ 풀이 탭이 유지되도록 문제 레이아웃 정리 로직을 보완했습니다.

## 0.0.16

- 확장 활성화 시점의 무거운 초기화를 실제 사용 시점으로 미뤄 사이드바가 더 빨리 뜨도록 개선했습니다.
- Webview 내부 로딩바를 제거하고, 사이드바 준비 중에는 VS Code 진행 표시를 사용하도록 정리했습니다.
- 문제 목록을 단일 인덱스 파일로 캐시해 목록 로딩 비용을 줄이고, 생성/삭제/다시풀기 변경 후 목록이 바로 갱신되도록 보완했습니다.
- `.programmers-helper` 내부 파일이 문제 목록에 섞여 보이지 않도록 필터링했습니다.
- 새 풀이와 초기화 시 전체 에디터를 다시 열지 않고 오른쪽 풀이 탭을 유지해 화면 깜빡임을 줄였습니다.
- 문제 전환 후 왼쪽에는 현재 문제 Markdown, 오른쪽에는 현재 풀이 C++만 남도록 탭 정리를 보완했습니다.
- 저장된 풀이 스냅샷을 실행할 때 현재 보고 있는 C++ 파일을 테스트 대상으로 사용하고, 컴파일 캐시에 소스 파일 경로를 포함해 잘못된 바이너리 재사용을 막았습니다.

## 0.0.11

- 샘플 테스트 실행 시 저장된 `programmers.json`의 예제를 우선 사용하고, 필요할 때만 `problem.md`에서 가져와 저장합니다.
- 사이드바의 샘플/커스텀 테스트는 명시적으로 열린 현재 문제가 있을 때만 실행되도록 했습니다.
- 확장 세션 동안 Docker 준비 확인 결과를 캐시하고, 캐시된 경로가 실패하면 새로 확인해 재시도합니다.
- 풀이 코드, 생성된 테스트 러너 코드, 컴파일 플래그가 바뀌지 않았으면 기존 컴파일 바이너리를 재사용합니다.
- 테스트 결과를 케이스별로 집계해 전체 출력 문자열을 누적하지 않도록 메모리 사용을 줄였습니다.
- 사이드바 문제 요약을 메모리에 캐시하고, 명시적 새로고침이나 파일 변경 동작 후에만 전체를 다시 스캔합니다.
- 출력 캡처 버퍼 수정으로 `clang++` 컴파일 오류 상세 출력이 다시 보이도록 복구했습니다.

## 0.0.10

- 사이드바에 문제별 `notes.md` 메모를 여는 동작을 추가했습니다.
- 새 메모 파일을 만들 때 문제 정보가 들어간 학습 노트 템플릿을 생성합니다.
- 메모 파일을 열어도 문제 Markdown 미리보기가 바뀌지 않도록 미리보기를 잠급니다.

## 0.0.9

- 사이드바의 문제 열기 / 현재 상태 섹션을 접을 수 있게 하고, 접힌 상태에서 상태 요약을 표시합니다.

## 0.0.8

- 테스트 실행 시간을 소수점 둘째 자리까지 표시하고 결과 줄에 메모리 사용량을 함께 표시합니다.
- C++ 컴파일과 테스트 실행을 Docker 런타임 컨테이너에서 수행합니다.
- 확장 런타임 코드를 역할별 모듈로 정리했습니다.
- 문제별 풀이 기록을 추가하고 새 풀이, 열기, 삭제, 초기화 흐름을 지원합니다.
- 저장된 풀이를 열어 둔 상태에서는 현재 보고 있는 C++ 파일을 대상으로 테스트를 실행합니다.
- 확장이 비활성화될 때 Docker 실행 컨테이너를 정리합니다.
- 현재 문제 관련 동작을 문제 목록 밖으로 옮기고 사이드바의 릴리스 메타데이터 표시를 개선했습니다.

## 0.0.7

- 로컬 설치 빌드와 분리된 환경에서 확장을 개발할 수 있도록 Dev Container 워크플로를 추가했습니다.
- 원격 및 컨테이너 세션에서 워크스페이스 확장 호스트에서 실행되도록 확장을 표시했습니다.
- 컨테이너 환경에 맞는 디버그 태스크와 실행 설정을 추가했습니다.
- 문제별 커스텀 테스트 케이스를 저장하고 문제를 다시 열 때 복원합니다.
- 사이드바에 문제 검색을 추가하고 테스트 동작 배치를 개선했습니다.
- 런타임 오류와 컴파일 실패를 구분하고 `clang++` 누락 안내를 더 명확하게 표시합니다.
- 긴 URL 때문에 레이아웃이 깨지지 않도록 사이드바의 긴 상태 메시지를 줄바꿈합니다.
- 휴지통을 사용할 수 없을 때의 문제 삭제 fallback 동작을 개선했습니다.

## 0.0.6

- 로컬 `Programmers` 폴더를 사용할 수 없을 때 VS Code 전역 저장소를 기본 문제 저장 위치로 사용합니다.
- 확인 절차와 휴지통 지원을 포함한 문제 폴더 삭제 동작을 추가했습니다.
- 활성 에디터 경로를 테스트 대상으로 사용하기 전에 검증합니다.
- 문제 페이지를 크롤링할 때 프로그래머스 리다이렉트와 응답 크기를 제한합니다.
- 긴 로컬 경로를 숨겨 컴파일 오류 요약을 더 읽기 쉽게 개선했습니다.
- C++ 테스트가 현재 사용자 권한으로 실행된다는 점을 문서화했습니다.

## 0.0.5

- 테스트 케이스별 제한 시간과 테스트 실행 중지 버튼을 추가했습니다.
- 전체 로그는 Output 패널에 유지하면서 사이드바에는 간결한 컴파일 오류 요약을 표시합니다.
- 제한 시간이 전체 테스트 묶음을 막지 않도록 생성된 테스트 케이스를 하나씩 실행합니다.
- 누락된 include가 가려지지 않도록 runner helper header보다 먼저 `solution.cpp`를 컴파일합니다.
- `Programmers/Programmers`를 만들지 않고 열려 있는 `Programmers` 폴더를 직접 사용합니다.

## 0.0.4

- 기존 로컬 문제에 이미 `problem.md`와 `solution.cpp`가 있으면 프로그래머스 크롤링을 건너뜁니다.
- 로컬 문제를 병렬로 불러와 문제 목록 새로고침 속도를 높였습니다.
- 더 이상 문제 파일이 없는 오래된 마지막 문제 경로를 무시합니다.
- 준비 상태와 테스트 완료 상태에 대한 사이드바 피드백을 단순화했습니다.

## 0.0.3

- 사이드바를 고정 영역, 테스트 영역, 문제 목록 영역으로 분리했습니다.
- 접을 수 있는 테스트 섹션과 문제 목록 섹션을 추가했습니다.
- 리뷰 토글을 문제 행 안으로 옮겼습니다.
- 준비 상태 텍스트와 간격을 줄였습니다.

## 0.0.2

- `.programmers-helper/review.json`에 저장되는 현재 문제 리뷰 체크박스를 추가했습니다.
- 사이드바에 접을 수 있는 리뷰 목록과 전체 문제 목록을 추가했습니다.
- 생성, 마지막 문제, 리뷰 목록, 전체 목록 동작에서 문제를 일관되게 열도록 개선했습니다.
- 문제를 열 때마다 커스텀 테스트 기본값과 준비 상태를 새로고침합니다.

## 0.0.1

- Programmers 사이드바 뷰를 추가했습니다.
- 프로그래머스 lesson 번호로 `problem.md`, `solution.cpp`, `programmers.json`를 생성합니다.
- 왼쪽에는 Markdown 미리보기, 오른쪽에는 C++ 풀이 파일을 엽니다.
- `problem.md`에서 파싱한 샘플 테스트를 실행합니다.
- Input / Expected Output 필드에 입력한 커스텀 테스트를 실행합니다.
